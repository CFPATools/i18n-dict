...
class PackerTests(unittest.TestCase):
    ...
    def test_load_config_supports_aggregated_upstream_structure(self) -> None:
        version = "1.21"
        tempdir = tempfile.TemporaryDirectory()
        self.addCleanup(tempdir.cleanup)
        root = Path(tempdir.name)
        write(
            root / "config" / "packer.json",
            json.dumps(
                [
                    {
                        "targetVersion": version,
                        "unTargetLang": ["en_us", "zh_cn"],
                        "modNameBlackList": ["blacklist-mod"],
                        "noProcessNamespace": ["font", "textures"],
                        "domainBlackList": ["guide"],
                        "additionalContent": ["README.md"],
                    }
                ],
                ensure_ascii=False,
                indent=2,
        ...
        self.assertEqual(config.base.version, version)
        self.assertEqual(config.base.target_languages, ("en_us", "zh_cn"))
        self.assertEqual(config.base.exclusion_mods, ("blacklist-mod",))
        self.assertEqual(config.base.exclusion_namespaces, ("font", "textures"))
        self.assertEqual(config.floating.exclusion_domains, ("guide",))
