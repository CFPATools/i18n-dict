...
def load_config(upstream_root: Path, version: str) -> Config:
    raw = json.loads(read_text(upstream_root / "config" / "packer" / f"{version}.json"))
    base = raw.get("base", {})
    floating = raw.get("floating", {})
    return Config(
        base=BaseConfig(
            version=base.get("version", version),
            target_languages=tuple(base.get("targetLanguages", [])),
            exclusion_mods=tuple(base.get("exclusionMods", [])),
            exclusion_namespaces=tuple(base.get("exclusionNamespaces", [])),
        ),
        floating=FloatingConfig(
            inclusion_domains=tuple(floating.get("inclusionDomains", [])),
            exclusion_domains=tuple(floating.get("exclusionDomains", [])),
            exclusion_paths=tuple(floating.get("exclusionPaths", [])),
            inclusion_paths=tuple(floating.get("inclusionPaths", [])),
            character_replacement=dict(floating.get("characterReplacement", {})),
            destination_replacement=dict(floating.get("destinationReplacement", {})),
        ),
    )
